from leapp.models import Report, OSReleaseFacts
...
def test_not_supported_id(current_actor_context):
    ...
    assert current_actor_context.consume(Report)[0].summary == 'Unsupported OS id'
...
def test_not_supported_major_version(current_actor_context):
    ...
    current_actor_context.run()
    assert current_actor_context.consume(Report)
    assert current_actor_context.consume(Report)[0].summary == 'Unsupported OS version'
