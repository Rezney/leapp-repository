...
OVERLAY_UNSUPPORTED = ('vfat',)
OVERLAY_DO_NOT_MOUNT = ('tmpfs', 'devpts', 'sysfs', 'proc', 'cramfs', 'romfs', 'sysv')
...
def _prepare_required_mounts(scratch_dir, mounts_dir, mount_points, xfs_info):
    result = {
        mount_point.fs_file: mounting.NullMount(_mount_dir(mounts_dir, mount_point.fs_file)) for mount_point in mount_points
    }
    unsupported = [
        mount_point.fs_file for mount_point in mount_points if mount_point.fs_vfstype in OVERLAY_UNSUPPORTED
    ]
    if not xfs_info.mountpoints_without_ftype and not unsupported:
        return result

    space_needed = _overlay_disk_size() * len(xfs_info.mountpoints_without_ftype + unsupported)
    ...
    for mountpoint in xfs_info.mountpoints_without_ftype + unsupported:
