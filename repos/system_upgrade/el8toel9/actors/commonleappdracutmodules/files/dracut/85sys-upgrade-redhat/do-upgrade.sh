...
NSPAWN_OPTS="--capability=all --bind=/sys:/hostsys --bind=/dev --bind=/dev/pts --bind=/proc"
[ -d /dev/mapper ] && NSPAWN_OPTS="$NSPAWN_OPTS --bind=/dev/mapper"
# POC:
export NSPAWN_OPTS="$NSPAWN_OPTS --bind=/run/udev --keep-unit --register=no --timezone=off --resolv-conf=off --console=pipe"
